...
@everywhere @model function model(; tracers_names=tracers_names,
                                    pairs=pairs,
                                    idx=idx,
                                    cov_tot=cov_tot, 
                                    files=files)
...
@everywhere (;data_vector) = rand(model() | (Ωm=0.30, Ωb=0.05, h=0.67, ns=0.96, s8=0.81,
                                 A_IA=0.1, alpha_IA=0.1,
                                 DESwl__0_e_nz_1=nz_k0[1], DESwl__0_e_nz_2=nz_k0[2], DESwl__0_e_nz_3=nz_k0[3],
...
# Start sampling.
folpath = "../chains"
folname = string("DES_wlwl_Nz0_toy_", "TAP_", TAP)
...
for i in (1+last_n):(last_n+cycles)
    if i == 1
        chain = sample(cond_model, NUTS(adaptation, TAP),
