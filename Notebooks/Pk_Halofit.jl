...
true_cosmology = LimberJack.Cosmology(0.25, 0.05, 0.67, 0.96, 0.81, tk_mode="EisHu", Pk_mode="Halofit")
true_data = true_cosmology.Pk(ks, 0.)
data = true_data + 0.1 * true_data .* rand(length(true_data))

@model function model(data)
    Ωm ~ Uniform(0.2, 0.3)
    cosmology = LimberJack.Cosmology(Ωm, 0.05, 0.67, 0.96, 0.8, tk_mode="EisHu", Pk_mode="Halofit")
...
CSV.write("Pk_Halofit_chain.csv", chain)
